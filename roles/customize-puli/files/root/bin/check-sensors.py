...
class SystemHealth:
    def __init__(self, out_dir="/vat/log/root/node_health", filename=None, spike_threshold=0.2, verbose=False):
    ...
    def detect_spikes(self, logs_dir=None, threshold=0.2):
        ...
        h2 = open(f"{self.out_dir}/critical_failure.log", 'a')
        n_spikes = 0
        n_critical = 0
        for node in self.nodes:
            node_data = self.data_to_assess[node]
            mean_df = pd.concat(previous_data[node]).groupby("Component")["Value"].mean()   # join all logs for that node, and get the average values
            std_df = pd.concat(previous_data[node]).groupby("Component")["Value"].std()     # join all logs for that node, and get the standard deviation
            percentages = (node_data.set_index("Component")["Value"] - mean_df) / mean_df
            spikes = percentages[abs(percentages) > threshold]
            critical_spikes = node_data[node_data["Value"] == "disabled"]
            for i in range(len(spikes)):
                spike_text = (
                    f"Node: {node}; Component: {spikes.index[i]}; "
                    f"Value: {'+' if spikes.iloc[i] > 0 else ''}{round(spikes.iloc[i] * 100, 2)} %; "
                    f"Mean: {round(mean_df[spikes.index[i]], 2)}; "
                    f"Std: {round(std_df[spikes.index[i]], 2)}")
                print(f"Spike detected! {spike_text}")
                h1.write(f"[{time.strftime('%Y-%m-%d_%H:%M:%S')}] {spike_text}\n")
            for i in range(len(critical_spikes)):
                critical_text = f"Node: {node}, Component: {critical_spikes.iloc[i]['Component']}, Value: {critical_spikes.iloc[i]['Value']}"
                print(f"Critical failure detected! {critical_text}")
                h2.write(f"[{time.strftime('%Y-%m-%d_%H:%M:%S')}] {critical_text}\n")
            n_spikes += len(spikes)
            n_critical += len(critical_spikes)
        print(f"Total number of spikes detected: {n_spikes}")
        print(f"Total number of critical failures detected: {n_critical}")
